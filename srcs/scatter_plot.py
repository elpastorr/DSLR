...
def compare_columns(data, col1, col2):
    ...
    if not pd.api.types.is_numeric_dtype(
        data[col1]
    ) or not pd.api.types.is_numeric_dtype(data[col2]):
    ...
    plt.scatter(
        ...
        data[col2],
        alpha=0.8,
        c=data["Hogwarts House"].apply(lambda x: colors[x]),
    )
...
def main():
    ...
    compare_columns(data, "Transfiguration", "Astronomy")
    compare_columns(data, "Defense Against the Dark Arts", "Astronomy")
    compare_columns(data, "Care of Magical Creatures", "Potions")
